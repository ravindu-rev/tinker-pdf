...
| A premultiplied-alpha option | alpha is straight only. **CMYK page output and the anti-aliasing switch landed** as `RenderOptions::allow_cmyk` and `RenderOptions::antialias`, each pinned in `crates/tinker-pdf/tests/render_options.rs` | a `RenderOptions` field with its own fingerprint | S |
