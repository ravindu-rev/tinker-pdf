...
| A premultiplied-alpha option; an anti-aliasing switch | alpha is straight only; no quality knob. **CMYK page output landed** as `RenderOptions::allow_cmyk`, pinned in `crates/tinker-pdf/tests/render_options.rs` | each a `RenderOptions` field with its own fingerprint; the switch changes no determinism claim | S each |
