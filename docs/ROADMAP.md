...
| **Memory has no measurement.** Thirty-seven caps are properties of inputs and none of a process; the two largest runtime bounds, `MAX_PAGE_PIXELS` and `MAX_DECODED_STREAM`, are not ledger rows; the only peak-memory tests are `#[ignore]`d and need an outside watcher | `crates/tinker-pdf/tests/bounds_ledger.rs`, `epub_memory.rs`, `xps_memory.rs` | the corpus runner records each child's peak resident set in `report.json` and the per-corpus maximum in `ratchet.json` as a `<=` band; the two caps get ledger rows with clock-free firing tests | M |
