...
| Metamorphic residue: 181 files fail `rotate`, 43 `crop`, 51 `dpi`, and the plausible causes are eliminated rather than confirmed. The residue is grouped and the population separated from the 273/293/45 files that *declined* each relation on the probe's wall clock; four identical differing-pixel counts cover 25 of the 51 `dpi` files, so those are one document reissued rather than 25 causes. `metamorphic_classes.rs` then ablates six constructs — a rectangle on integers, diagonal path edges, an isolated transparency group at half alpha, an axial shading, an image at a non-integer scale, and text — and **all six move zero pixels**, which rules each of them out. The corpus's failures move 20 % to 42 % of a page, and no edge effect is a fifth of a page | `crates/tinker-pdf/tests/metamorphic_classes.rs`, whose zeros are asserted; the grouping in its header | the cause bisected from a real witness rather than a synthetic guess, then attributed by class in a successor to [design/image-edges.md](design/image-edges.md), or fixed. `dpi` on every file is a stated non-goal there | M |
