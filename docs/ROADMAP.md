...
The suite stands at 4 520 passed, 0 failed, 56 ignored as
...
| **The PDF/A validator's 37 staged rules.** Agreement with the veraPDF corpus is 1 476 of 2 371 PDF/A files — 830 of 831 `pass`, 646 of 1 540 `fail`. The unagreed `fail` files fall into families: 247 on the **membership** half of the XMP predefined-schema rule, whose value-type half now runs and took 265 files with it, 250 in graphics clauses that run and not far enough, 90 in font clauses waiting on code-to-glyph mapping, 142 in annotation clauses that have no rule group at all; then Level A's structure rules, content-stream rules as an interpreter rather than a tokenizer, recursive validation of embedded files, the output intent's own ICC conformance, and 6.11 and 6.12 with fixtures and no rules | `PDFA_STAGED` in `crates/tinker-pdf/src/pdfa.rs`, counted; [features/pdfa.md](features/pdfa.md); [design/pdfa.md](design/pdfa.md) | the staged count moves down and the agreement ratchet up, one ledger class at a time | L ([design/pdfa.md](design/pdfa.md)) |
