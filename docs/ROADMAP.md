...
| Item | Evidence | Exit criterion | Size |
| --- | --- | --- | --- |
| The aggregate: **2 of the 118** JBIG2-bearing corpus files still report a refusal, down from 34, and **every real-world document in five corpora decodes**. Halftone, custom code tables, transposed placement, 7.2.7's unknown data length, a blank page, 7.4.2's retained bitmap-coding contexts, a text region that places nothing and 6.5.8.2.2's reference offset have all left. What is left is one pdf.js fixture waiting on the three short Annex B tables and one that stops inside a segment | `jbig2_attribution.rs`, whose counts are pinned and which names both files | the number, re-measured after each row | — |
