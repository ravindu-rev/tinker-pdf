...
The suite stands at 4 968 passed, 0 failed, 59 ignored across 220 suites as
...
- **No producer exists here** for a fixture, so the third route — build it and
  ...
  **And it stretches further than one codestream, which was the surprise of
  ...
  needed no hand-authored codestream at all. Whether J.10 can be re-expressed
  for RGN, POC, `BYPASS` or `TERMALL` is a different question with, so far, a
  different answer — none of those four can be reached by rearranging a
  codestream that does not use them.
  ...
  **The fifth search came back empty, and it is the useful one.** On 20
  ...
  pattern, and POC, PPM, PPT, `BYPASS` and `TERMALL` should each be asked
  whether it applies before anything is hand-authored.
...
| **JPX: the POC marker and the `BYPASS` and `TERMALL` code-block styles — three capabilities, and the row does not close until all three land.** **PPM and PPT have left this row**: packed packet headers (T.800 A.7.4, A.7.5) are implemented, in both places the mechanism lives, through tier-2's one packet-header reader with the byte source parameterised rather than a second reader. **RGN has left it too, the same day**: A.6.3's marker segment is parsed at Table A.24's widths and precedence, and Annex H.1's Maxshift realignment runs at the head of the dequantiser, where D.2.2 puts it — "modifications need to be made to the decoded bits, as well as the number of decoded bits `Nb(u, v)` … specified in H.1". The scope is the whole of what Part 1 defines: Table A.25 gives one ROI style, 0, and reserves the rest, so style 0 decodes and a reserved style is refused **by name** as a reserved style rather than stepped over — the SOF3/SOF5/SOF6/SOF7 lesson two rows down, where a skipped frame type surfaced a lossless JPEG as a damaged file. **Component precision above 16 bits has left this row and is a limit**, argued under Named non-goals: E.1's dequantisation clamps a coefficient to `2^(R_b + 2)` sample units and the coefficient plane is a Q12 `i32`, so 17 bits is where the plane's format runs out rather than where a policy begins — and ISO 32000-1 Table 89 has no `/BitsPerComponent` above 16 to hand a widened sample to. CRG has left this row for a different reason: ISO 19005 aside, T.800 A.9.1 says in as many words that it "has no effect on decoding the codestream", so it is parsed, carried and never applied, and refusing a file for carrying it refused a conforming file | **Zero corpus files reach any of them, and closing RGN, PPM and PPT did not move that.** `jpx_attribution.rs` names every refusal in the 39 JPX-bearing files and not one is a coding capability: a ruling 1 budget, two deliberately non-conformant veraPDF `colr` fixtures, two `/JPXDecode` streams whose bytes are not JPEG 2000 at all, and two real documents that are truncated. Re-run after each of the three landed, the census was unchanged — same 39 files, same five reasons — which is what closing an unreached capability should do | **one blocker left of three, and the row is scheduled.** (1) Zero corpus reachability still holds, and under ruling 3 that is a scheduling input rather than a wall. (2) **T.800 is obtainable** and was fetched on 13 September 2026 — the claim that ISO and the ITU both sell it was never retested; B.10.7.2's rule for how many codeword segments a packet signals is at hand rather than written from memory. (3) **A fixture no longer needs an encoder**, and for packed packet headers it did not even need to be hand-authored: T.800 Annex J.10 publishes a complete codestream with its decoded samples, and `jpx_annex_j.rs` decodes it — **and, in J.10.3 and J.10.4, where each packet's header ends**: Table J.20's three bytes with its body at octal 0125, Table J.21's four bytes with its body at octal 0137, so those headers can be relocated into a PPM and into a PPT and judged by J.10.5's published samples. What the standard does *not* publish is a codestream exercising POC, `BYPASS` or `TERMALL`, so each still needs a hand-authored one transcribed from its own clause — and the first measurement of that work says how carefully: of 182 citations drafted from T.800 for these seven capabilities, **37 were wrong** and only one of six specifications came back clean. **For ROI that "does not publish" was tested rather than assumed, and this time it held**: every annex was searched, Annex H is prose and seven equations with no worked example and no sample value, K.4 is five bibliography entries, J.9 mentions ROIs only as a way of interpreting multi-component SAR data, and `0xFF5E` occurs exactly twice in the 231 pages — Table A.2 and Table A.24 — so no codestream the standard prints carries an RGN. What made RGN adjudicable anyway is that **J.10 publishes its intermediate coefficients**, not only its samples: J.10.4 states "-26, -22, -30, -32, -19" and "1, 5, 1, 0", which are exactly what H.1 rewrites, so `jpx_annex_h.rs` runs the clause over the standard's own numbers and demands J.10.5's samples back. That trick is available to any capability that acts on coefficients | scheduled; one capability at a time, each with the clause it is transcribed from checked against T.800 twice |
...
| Item | Today | Exit criterion | Size |
| --- | --- | --- | --- |
| A user-facing CLI | nine subcommands, all read-only; the write half of the library is unreachable from it | merge, split, rotate, images, fonts, encrypt, decrypt, sign, attach, stamp, sanitise — each a wrapper over the facade with no logic of its own (ruling 11) | M |
