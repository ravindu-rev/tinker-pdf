...
The suite stands at 4 550 passed, 0 failed, 56 ignored as
...
| **The PDF/A validator's 37 staged rules.** Agreement with the veraPDF corpus is 1 624 of 2 371 PDF/A files — 830 of 831 `pass`, 794 of 1 540 `fail`. The unagreed `fail` files fall into families: 247 on the **membership** half of the XMP predefined-schema rule, whose value-type half now runs and took 265 files with it, 250 in graphics clauses that run and not far enough, 97 in font clauses waiting on code-to-glyph mapping; **the annotation clauses have a rule group now** and not one of their files still disagrees, which with the interactive-form rows it unblocked moved 148; then Level A's structure rules, content-stream rules as an interpreter rather than a tokenizer, recursive validation of embedded files, the output intent's own ICC conformance, and 6.11 and 6.12 with fixtures and no rules | `PDFA_STAGED` (the `lib.rs` re-export of `STAGED` in `crates/tinker-pdf/src/pdfa.rs`), counted; [features/pdfa.md](features/pdfa.md); [design/pdfa.md](design/pdfa.md) | the staged count moves down and the agreement ratchet up, one ledger class at a time | L ([design/pdfa.md](design/pdfa.md)) |
...
| **Non-device colour spaces on write.** **The ICC half is half-built and this row used to deny it existed.** `DocumentBuilder::add_icc_color_space` writes 8.6.5.5's `[/ICCBased stream]` with `/N` validated against Table 66, and the XPS painter has been its consumer for months. What is absent is a typed `PageBuilder` setter — the operator emitter exists privately as `context_colour_operators`, which already pads operands to the space's `/N` and reaches the page through `PageBuilder::raw` — and an ICC variant on `add_image`'s `ImageColorSpace`, which is the only thing XPS's `{ColorConvertedBitmap}` refusal waits on. CIE, `/Separation` and `/DeviceN` are genuinely absent | `add_icc_color_space` in `crates/tinker-pdf-cos/src/build.rs`; its consumer in `crates/tinker-pdf/src/xps/profiles.rs`; [features/xps.md](features/xps.md) | a typed setter and an `ImageColorSpace` variant close the XPS row; `/Separation` and `/DeviceN` need a tint-transform writer | S for the ICC half, M for the rest |
