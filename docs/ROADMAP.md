...
| Item | Evidence | Exit criterion | Size |
| --- | --- | --- | --- |
| The aggregate: **3 of the 118** JBIG2-bearing corpus files still report a refusal, down from 34. Halftone, custom code tables, transposed placement, 7.2.7's unknown data length, a blank page, 7.4.2's retained bitmap-coding contexts and a text region that places nothing have all left. What is left is one file waiting on the three short Annex B tables, one stream that stops inside a segment, and one real document | `jbig2_attribution.rs`, whose counts are pinned and which names all three files | the number, re-measured after each row | — |
