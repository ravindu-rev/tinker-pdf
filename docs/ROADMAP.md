...
| **The PDF/A validator's 38 staged rules.** Agreement with the veraPDF corpus is 1 781 of 2 371 PDF/A files — 830 of 831 `pass`, 951 of 1 540 `fail`. The unagreed `fail` files fall into families: **conformance level A is validated now** — `/MarkInfo`, the structure tree root, every structure type after the `/RoleMap`, and every `/Lang` the object graph carries, which took 19 of the 21 logical-structure fixtures and left `830 of 831` where it was; then 168 files whose defect is an XMP value *type* finer than the four forms an RDF/XML serialisation distinguishes (`Integer` and `Rational` and `Date` are all one element with text in it), four on an extension schema's custom value types, and six on the packet header's bytes and encoding attributes, a /Metadata stream that carries a filter, and a packet Isartor calls malformed; then 197 in graphics clauses that run and not far enough — the content-stream operator list, the Separation tint transform and the destination profile's own conformance, none of which is a prohibition over a dictionary — 97 in font clauses waiting on code-to-glyph mapping, content-stream rules as an interpreter rather than a tokenizer (which is what level A's two remaining logical-structure fixtures wait on, and 6.2.11.7.3's five `/ActualText`-per-character ones with them), recursive validation of embedded files, the output intent's own ICC conformance, and 6.11 and 6.12 with fixtures and no rules | `PDFA_STAGED` (the `lib.rs` re-export of `STAGED` in `crates/tinker-pdf/src/pdfa.rs`), counted; [features/pdfa.md](features/pdfa.md); [design/pdfa.md](design/pdfa.md) | the staged count moves down and the agreement ratchet up, one ledger class at a time | L ([design/pdfa.md](design/pdfa.md)) |
...
| **Editing.** Redaction cuts a rotated or skewed run along its own baseline; what it still refuses to *measure*, and therefore leaves whole and names in `RedactionReport::warnings`, is a **vertical writing mode** (9.4.4 advances by `/W2`'s `w1` down the page and a `TJ` number displaces along that axis, which is a different formula rather than a different matrix) and a **Type 3 font whose `/FontMatrix` is not the 1/1000 default** (9.6.5 puts its `/Widths` in a glyph space `width / 1000` does not map out of). The other two warning classes — a `Tf` naming a font the resources in scope do not have, and a text rendering matrix whose composition is not finite — are permanent and live in the feature doc's refusal table, not here, because neither has an exit: a run with no metrics has no positions to compute. Text inside a Type 3 glyph procedure or an annotation appearance stream is not rewritten; appearance synthesis covers seven annotation subtypes | [features/editing.md](features/editing.md); the four variants of `RedactionWarning`, each with a test of its own in `crates/tinker-pdf/src/redact.rs`'s `refusals` module | a vertical run measured by `/W2` and cut; a Type 3 font's glyph space read from its `/FontMatrix` rather than assumed; `/AP` and glyph-procedure streams rewritten; an `/AP` per further subtype | S; S; M; S per subtype |
| **A form XObject drawn twice is redacted only at its first placement.** `redact::apply` rewrites each form once — the `visited` set that keeps a self-referential form from recursing makes the second `Do` a no-op as well, which is right when both invocations share a transform and wrong when they do not. A rectangle over the second placement is never tested against it: the text stays, and the report cannot be told from a rectangle that covered nothing, because `glyphs` is 0 and `warnings` is empty. It is the one under-redaction this module does not name, which is why the row above claims the rotated cut and not redaction as a whole | `a_form_drawn_twice_is_cut_only_at_its_first_placement` in `crates/tinker-pdf/src/redact.rs`, which asserts the defect rather than the fix, with its reasoning in its own doc comment; [features/editing.md](features/editing.md) | either a copy of the form per `Do`, rewritten in that placement's frame, or a fifth `RedactionWarning` when one form is invoked under two transforms — and the pin flipped to assert it | S |
