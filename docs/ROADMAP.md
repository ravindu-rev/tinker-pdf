...
| The aggregate: **5 of the 118** JBIG2-bearing corpus files still report a refusal, down from 34. Halftone, custom code tables, transposed placement, 7.2.7's unknown data length and a blank page have all left. What is left is one retained context (below ruling 3's line), one text region whose dictionary exports nothing, one file waiting on the three short Annex B tables, one truncated stream, and one real document | `jbig2_attribution.rs`, whose counts are pinned | the number, re-measured after each row | — |
| **`safedocs/0000337.pdf` does not decode**: 46 pages of real OCR, every page refused, and the refusal differs per page — an aggregate instance count, a width, a refined size, a symbol index, more symbols than declared. That is one desynchronisation presenting five ways rather than five defects | Narrowed to the **refinement/aggregate symbol dictionary** (`SDREFAGG = 1`, `SDRTEMPLATE = 1`): the plain dictionary before it decodes and exports its 1 430 symbols, the aggregate one reads a sane height, width and instance count and then loses step inside the first refined symbol. Two readings of 6.5.8.2 were measured and 6.5.8.2.2's shortcut is the better one — routing the single instance through 6.4's strip loop instead decodes *fewer* pages, not more. `bitmap-symbol-symbolrefineone-template1.pdf` covers the same flag combination and passes, so what is unexercised is the combination **with a large imported set**: `SBSYMCODELEN` is 11 here and 2 there | the 46 pages decode; a fixture that isolates the aggregate road at a symbol-code width above eight | M |
| **Three of Annex B's fifteen tables are not complete prefix codes**: B.7 at 0.640 of one, B.10 at 0.945, B.12 at 0.921. B.3 assigns canonical codes from prefix lengths alone, so below Kraft equality some bit patterns decode to nothing, `HuffTable::decode` returns `None`, and the file is refused with no reason recorded. B.7's lower line was also written as an *upper* one — counting up from −1025 where it should count down — and that half is fixed | `annex_b_tables_that_are_not_complete_prefix_codes_are_named` pins all three sums; `bitmap-symbol-symhuffB5B3-texthuffB7B9B12.pdf` is the file that pays for it. B.11 and B.13 are the same family as B.12 and are complete at thirteen lines where it has ten, so what is missing is lines rather than a changed length | the three reconstructed and the pin emptied. **Not by a table that looks plausible**: this module's record has five refinement templates that decoded one fixture into legible text and were all wrong. What can settle it is the file itself — its picture is known from `bitmap-symbol.pdf`, so the value sequence the text region must decode is known, and the bit stream then determines the codes for the lines it uses | M |
| ICC profiles this build cannot make a transform of: the `A2B*` table of a v4 `mAB` profile, a connection space other than XYZ, a data space with no transform here | **8 of the corpus's 3 235 profiles**, re-measured over five corpora in September 2026 — 4 a connection space, 3 a data space, 1 the `mAB ` tag. The figure this row used to carry, 143 of 2 750 in 131 files, paired a four-corpus denominator with a different measurement ([features/rendering.md](features/rendering.md), [design/icc.md](design/icc.md)) | the count falls; what is still refused is refused by name with its count, and the `iccbased` capability floor now fails a build that stops seeing them | M ([design/icc.md](design/icc.md)) |
| **`CalRGB` and `CalGray` are aliased to `DeviceRGB` and `DeviceGray`**, not approximated: 8.6.5.1 and 8.6.5.2's `/WhitePoint`, `/Gamma`, `/Matrix` and `/BlackPoint` are never read, and no type records that an approximation happened — so unlike a refused ICC profile this one is invisible | `parse_space` and its two siblings in `crates/tinker-pdf/src/resources.rs`; [features/rendering.md](features/rendering.md) | either the two spaces convert through their own white point and gamma, or the aliasing is recorded on the type the way `Approximated` is | S |
...
| JPX: the RGN, POC, PPM, PPT and CRG markers, the `BYPASS` and `TERMALL` code-block styles, component precision above 16 bits, tile-parts out of order | **5 of the 39** JPX-bearing corpus files report something. The row used to conclude that "nothing here is reached by a real document" while accounting for only three of the five: one is a budget (`pdfjs/issue19517`) and two are deliberately non-conformant veraPDF fixtures, but **`safedocs/0000314` and `safedocs/0000591` are real documents off the open web**, both refused under `JpxStructureInvalid` — which is one warning over four distinct `Refusal::Structure` conditions, so which of them these two hit is not yet known | the two attributed to a named condition, the way the JBIG2 files were; then scheduled or not on that evidence | S to attribute; `BYPASS`/`TERMALL` are M, needing a length per coding pass out of the packet header (B.10.7) |
