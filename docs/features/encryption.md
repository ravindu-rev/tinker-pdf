...
workspace suite: **4 655 passed, 0 failed, 56 ignored across 212 suites**,
measured on this branch on 15 September 2026 with Triple DES landed (Windows
x86_64) — 15 more than the 4 640 the rest of this tree still records, which is
this commit's fourteen `des.rs` tests and the one in `pubsec.rs`. Other work
is in flight against the same total, so the figure to trust is whichever one
carries the later date — [verification](../verification.md).
