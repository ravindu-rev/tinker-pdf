...
sweeps the same shapes on stable. `tools/oracle-diff` runs `mutool draw -F
txt` as a subprocess text oracle (ruling 9). Across the corpus, 4 484 of
4 525 files rendered every page with 0 crashes, and `cargo test
--workspace` stands at 2 787 passed / 0 failed / 8 ignored (Windows x86_64,
as of August 2026).
