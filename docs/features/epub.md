...
| Structure elements spanning pages | each page's roots are wrapped separately, which is what stops logical order from repairing the float row above |
...
| A float that reads out of order across a page break | `tinker_pdf_layout::Warning::FloatBrokenAcrossPages`, and the pinned `Why::Reordered` book in `epub_fetched.rs` | **Two halves, one closed.** The forced-break half is closed: `fragment::beside` asked whether a float was within `top + page height` of a page's top, which is right only when a page is as tall as the page box, and a page ended by `page-break-before: always` is not — the reach ran into the next page's column and drew a float before the heading it was written after. Fixing it took both Waste Land spellings to **0** and `pg16328-beowulf.epub` from 4 560 to **2 182**, with no other book moving; `epub_float_order.rs` holds the shape as a book this repository builds. **What remains is §9.5.1 placing a float below text that follows it**, which `clear` does whenever one gloss clears the one before it. Inside a page it costs nothing — runs are sorted by their reading-order stamp. Across a page boundary it cannot be repaired by ordering, and **that has now been measured at the logical layer too, not only the geometric one**: with a structure tree emitted, extraction in ISO 32000 §14.8 logical order gives Beowulf the same **2 182**. A structure element's marked-content kids live on the page their `BDC` was written on, and `DocumentBuilder` wraps each page's roots in one `/Document` in page order, so logical order *is* page order then reading order — already what content order is here. Closing it needs a structure element whose kids span pages, which `PageBuilder` builds per page and cannot express. Four fixes have been built and measured against this row and none moved the number; each is named in `epub_fetched.rs` | [ROADMAP.md](../ROADMAP.md) |
