...
| A float that reads out of order across a page break | `tinker_pdf_layout::Warning::FloatBrokenAcrossPages`, and the pinned `Why::Reordered` book in `epub_fetched.rs` | **The mechanism, measured.** CSS 2.2 §9.5.1 places a float by geometry — `clear` puts it under the float before it, rule 7 stacks it beside one it does not fit next to — so a float's box can end up below text that follows it in the source. Inside one page that costs nothing: a page's runs are sorted by their reading-order stamp, and the stamp is source order whatever the `y`. Across a page boundary it cannot be paid — the float's box is on one page, its text is its box's, and there is no third place to put it. A second, narrower cause was found and a fix for it built: `fragment::beside` asks whether a float is within `top + page height` of a page's top, which is the wrong question after a **forced break**, because such a page ends early and the reach runs into the next page's column. Tightening it closes `sample-wasteland-otf-obf.epub` (3 characters to 0) and halves `pg16328-beowulf.epub` (4 560 to 2 182) — and takes `sample-trees.epub` from **exact conservation to 628**, because that book's floated sidebar wants the opposite answer from the same rule. Narrowing to forced breaks alone does not separate them; nor does deciding the page by the float's static position rather than its placed one. Both were built and measured, and neither shipped. **What closing this needs** is a float's text to be separable from its box across a page break: the box drawn where §9.5.1 put it, the runs stamped onto the page a reader meets them on — a change to what a `FloatRecord` is, not to where `beside` looks | [ROADMAP.md](../ROADMAP.md) |
