...
| A float that reads out of order across a page break | `tinker_pdf_layout::Warning::FloatBrokenAcrossPages`, and the pinned `Why::Reordered` book in `epub_fetched.rs` | **Half of this closed, and the half that is left is a different sentence.** What closed was a page's *reach*: `fragment::beside` asked whether a float was within `top + page height` of a page's top, which is the right question only when a page is as tall as the page box. A page ended by a **forced break** is not — `page-break-before: always` on a chapter heading ends a page anywhere — and the reach ran past the break into the next page's column, drawing a float before the heading it was written after. Fixing it took both Waste Land spellings from 3 characters to **0** (their rows are deleted) and `pg16328-beowulf.epub` from 4 560 to **2 182**, with no other book moving. `epub_float_order.rs` holds the shape as a book this repository builds, so it no longer depends on a fetched corpus. **What remains** is §9.5.1 placing a float *below* text that follows it in the source, which `clear` does whenever one gloss clears the one before it. Inside one page that costs nothing: a page's runs are sorted by their reading-order stamp, which is source order whatever the `y`. Across a page boundary it cannot be paid — the float's box is on one page, its text is its box's, and there is no third place to put it. **Three fixes were built and measured against it and none moved the number**, which is worth more than the row's earlier guess: deciding a float's page by where it was *met* rather than where it was placed (a `static_top` on `FloatRecord`) changed nothing at all, because the box still draws where §9.5.1 put it and a glyph is only on the page it is drawn on; disabling `css-break-3`'s push made this book **sixty times worse**, 2 182 to 136 966; and tightening the reach further is what already closed the forced-break half. So the remaining defect is not a stamping problem and not a page-assignment problem: it needs a float's **glyphs** on a different page from its box, which a PDF's text layer cannot express, or a placement that keeps the box on its reading page and overlaps the float above it, which §9.5.1 rule 3 forbids and this crate's `rule_3_*` fixture holds | [ROADMAP.md](../ROADMAP.md) |
