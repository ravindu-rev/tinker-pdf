...
| --- | --- | --- | --- |
| Any image encoder but deflate | `Rgb8`/`Gray8` are deflated; JPEG and PNG-IDAT pass through; nothing is encoded to JPEG, CCITT, JBIG2 or JPX | **The reason changed on 15 September 2026 and the refusal did not.** It used to be "the engine decodes those codecs; it does not write them"; `tinker-pdf-filters` now writes CCITT G4 and JBIG2 generic regions, and this builder calls neither. What is missing is not a coder, it is the decision of *when* a raster is better off as a fax coding than as deflate — and for JBIG2, D.3's embedded-stream assembly, which the filter crate deliberately does not write | [filters](filters.md), [ROADMAP](../ROADMAP.md) |
