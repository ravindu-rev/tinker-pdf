...
**Ten leaf crates** — `filters`, `crypto`, `font`, `color`, `raster`, `math`, `zip`, `xml`, `css`, `layout` — are bytes-in/values-out with zero PDF types. This is the property that makes each one independently fuzzable: a fuzz target hands `tinker-pdf-font` a byte slice and expects a value or a structured error, with no COS machinery in the corpus or the crash triage. It also means a leaf can be tested against its spec (DEFLATE against RFC 1951, CFF against Adobe TN 5176) without a PDF in sight.

*Amended, August 2026, twice, and the second time is why the amendment is dated in place rather than folded in silently.* This said "five" from the day it was written. `tinker-pdf-math` arrived with ruling 4's amendment as a **second-order leaf** — `no_std`, depended on by `color` and `raster`, depending on nothing — and not one of the three prose statements of the count moved, in this file, in [99-consistency](99-consistency.md)'s ruling 8, or in [CONTRIBUTING.md](../../CONTRIBUTING.md)'s rule 3. It had drifted for weeks before gap 29 went looking. `tinker-pdf-zip` is the seventh, added by that gap for CBZ and reused by gap 30 for XPS, and the sweep that added it is the one that found the first drift. A count written in four places and enforced in none is a fact about the documentation rather than about the code — which is why the count is stated here with the crates enumerated, so that a reader can check it against `crates/` rather than take it.

*Amended a third time, 22 August 2026, by gap 31 milestone 13.* **Eight to ten**, and this is the sweep the amendment above asked for arriving on schedule rather than after a drift. `tinker-pdf-css` is the ninth and is the fourth crate in the workspace with **no internal dependency at all** — which is not decoration: its allow-list being empty is what makes gap 31's decision 5 provable at compile time, because a `const` block can only name constants its own crate can reach. `tinker-pdf-layout` is the tenth and depends on `css` alone, which is the third leaf-to-leaf edge after `font -> filters` and `zip -> filters` and is acceptable on the same three grounds. It is also **the only leaf here whose input is not bytes**: a caller hands it a tree of plain structs, so it is a leaf on the definition rather than on the shape, and its fuzz target drives a structured generator with no parser in front of it. That is why `MAX_BOX_DEPTH` exists where gap 31's own bounds table argued a depth cap could not.

*Amended again, 19 August 2026, gap 30 milestone 9.* `tinker-pdf-xml` is the **eighth**, added by that gap's milestone 2 for XPS's fixed-page markup and named as the crate [31](README.md), EPUB, reuses — while reusing **none** of gap 30's package layer, because EPUB's container is OCF rather than OPC. It is the third crate in the workspace with no internal dependency at all, beside `filters` and `crypto`, and `xtask`'s `ALLOWED` doc comment argues the empty allow-list as a *finding* rather than an omission. Two amendments in two gaps is the drift this paragraph predicted; the count moved in all four places this time, in the milestone that owed the sweep.

Two of the eight have a dependency, and both are leaf-to-leaf: `font -> filters` and `zip -> filters`, each argued in `xtask`'s `ALLOWED` doc comment. A leaf here means bytes in, values out, no PDF types, independently fuzzable — not "no edges". The graph cannot cycle, because `filters` depends on nothing.
