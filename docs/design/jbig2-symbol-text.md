...
- **Halftone regions and pattern dictionaries** (clauses 6.6, 6.7; segment
  types 16, 20, 22, 23) — a third lineage with near-zero corpus presence; they
  keep refusing under `Jbig2SegmentSkipped`.
...
- **Retained bitmap-coding contexts across segments** (the used/retained flags
  in the symbol dictionary flags field, 7.4.2) — counted by the census below
  and refused by name if the count stays zero.
...
**Measure before staging** (ruling 3). Milestone 1 is a census over the 103
corpus files: an `#[ignore]`d test parses segment headers and tallies SDHUFF,
SBHUFF, SDREFAGG, SBREFINE and context-retention flags. `jbig2enc` emits only
arithmetic coding, so the expectation is arithmetic ≫ Huffman ≫ refinement —
but the order of milestones 5 and 6 is decided by the tally, not the
expectation, and the numbers are recorded in this document when known.
...
|---|---|---|---|
| 1 | Corpus census of the 103 JBIG2 files | `#[ignore]`d census test prints per-file SDHUFF/SBHUFF/SDREFAGG/SBREFINE/retention tallies; numbers recorded here; milestone 5/6 order fixed by them | S |
| 2 | Segment references + Annex A integer decoders | `Segment` carries number and referred-to list; IAx/IAID round-trip against `MqEncoder`-built sequences covering every A.2 prefix range and the OOB value; existing fuzz seeds still clean | M |
