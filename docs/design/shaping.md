...
| 5 | USE shaper for Indic and Southeast Asian scripts | Every text-rendering-tests USE section passes, counted; scripts with no fixture are listed by name in the feature doc as shaped-but-unverified rather than counted as done. **Not met, and not relaxed**: seven of sixteen sections pass whole and 301 of 333 cases are reproduced, pinned exactly by `PASSING`; `TRIAGE` says of every one of the thirty-two that do not whether the glyph *set*, their *order* or only a *position* is wrong; the shaped-but-unverified list is in [features/fonts.md](../features/fonts.md) | XL |
...
| Risk | Mitigation |
|---|---|
| Indic/USE correctness is effectively unbounded — the reason this is the roadmap's largest single item | USE's data-driven cluster model rather than per-script shapers; the Unicode and aots conformance files gate what they cover; scripts scheduled by corpus evidence under ruling 3, not completeness. **Milestone 5 stopped where that mitigation runs out**, and the place is now measured rather than described: Kannada's `SHKNDA-2/1` and Tai Tham's `SHLANA-2/6` are the same cluster shape and want opposite glyph orders, no property this crate reads separates them, and the rule that satisfies one costs sixty-nine cases of the other — the per-section numbers are in `crates/tinker-pdf-shape/src/universal.rs` |
