...
Numbers on this page were measured in August 2026, except the fuzz sessions and the corpus attributions below, which are 5-6 September 2026, and the suite total, which is 16 September 2026. `cargo test --workspace`
is **4 758 passed, 0 failed, 56 ignored** across 215 suites on
`x86_64-pc-windows-msvc`. That total was measured on the branch that added
the baseline JPEG encoder, which was cut from a commit behind `develop`, so it
counts that branch's seventeen new tests and not whatever landed on `develop`
beside them; it is a floor rather than the merged figure. The same suite was 2 243 passed, 0 failed on
